...
opt = {
   ...
   gpu_index = 0,                -- gpu = 0 is CPU mode. gpu=X is GPU mode on GPU X
   name = 'coco',
...
local input_segment2 = torch.Tensor(opt.batchSize, 1, opt.fineSize, opt.fineSize)

-- local input_image = torch.Tensor(opt.batchSize, 3, opt.fineSize, opt.fineSize)
...
local sample = function()
  ...
  real_seg, wrong_seg, real_img = data:getBatch()
...
local fDx = function(x)
  ...
  netD:backward({input_segment, input_image}, df_do)
  ...
  if opt.cls_weight > 0 then
    ...
    local output = netD:forward{input_segment2, input_image}
    errD_wrong = opt.cls_weight*criterion:forward(output, label)
    local df_do = criterion:backward(output, label)
    df_do:mul(opt.cls_weight)
    netD:backward({input_segment2, input_image}, df_do)
  end
