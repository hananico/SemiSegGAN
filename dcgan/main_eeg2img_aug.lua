...
opt = {
   ...
   cls_weight = 0.05,
   filenames = '',
   data_root = '/home/student/EEG2/new_data/EEGClassesAll',
   checkpoint_dir = '/home/reedscot/checkpoints',
	 batchset = './eeg_pairs_batch.txt' ,
   batchSize = 64,  -- 64 -> 10
   loadSize = 64,  -- 256 -> 64
   fineSize = 64,  -- 256 -> 64
   txtSize = 128,         -- #  
   nt = 100,               -- #  of dim for text features.
   nz = 100,               -- #  of dim for Z
...
-- local input_eeg = torch.Tensor(opt.batchSize, 3, opt.fineSize, opt.fineSize)
local input_eeg = torch.Tensor(opt.batchSize, opt.txtSize)
...
local sample = function()
 ...
 real_img, wrong_img, real_eeg = data:getBatch()
 --print('getbatch done')
 --  real_seg, wrong_seg, real_eeg = data:getBatch()
...
local fDx = function(x)
  ...
  local output = netD:forward{input_img, input_eeg}
 
  local errD_real = criterion:forward(output, label)
  local df_do = criterion:backward(output, label)
  netD:backward({input_img, input_eeg}, df_do)
  ...
  if opt.cls_weight > 0 then
    ...
    local output = netD:forward{input_img2, input_eeg}
    errD_wrong = opt.cls_weight*criterion:forward(output, label)
    local df_do = criterion:backward(output, label)
    df_do:mul(opt.cls_weight)
    netD:backward({input_img2, input_eeg}, df_do)
  end
  ...
  local fake = netG:forward{noise, input_eeg}
  input_img:copy(fake)
...
local fGx = function(x)
  ...
  local df_dg = netD:updateGradInput({input_img, input_eeg}, df_do)
 
  netG:backward({noise, input_eeg}, df_dg[1])
