...
batch_size = 1
local data_img_seq1 = torch.FloatTensor(batch_size, 3, loadSize[2], loadSize[2])
...
cutorch.setDevice(1)
manualSeed = torch.random(1, 10000) -- fix seed
...
--net =net:cuda()
--output = net:forward(data_img_seq1:cuda(), data_lbl_seq1:cuda())
--n_class = 33
--print(output:size())

--caffemodel = loadCaffeModel()
--print(caffemodel:size())
--caffemodel = caffemodel:cuda()
folder = '/home/student/caffe_orig/caffe-master/VOC/'
prototxt =  folder .. '/deploy.prototxt'
caffemodel_path = folder .. '/snapshots/voc_fcn8_train1400_cls22_iter_10000.caffemodel'
caffemodel = caffe.Net(prototxt, caffemodel_path, 'test')
--caffemodel = caffemodel:cuda()
output = caffemodel:forward(data_img_seq1)
--local torch_parameters = net:getParameters()
--torch_parameters = torch_parameters:cuda()
print ( output:size())
--print (torch_parameters)
local caffeparameters = caffemodel:getParameters()
--caffeparameters = caffeparameters:cuda()
--print ("caffeparameters")
print (caffeparameters:size())

--torch_parameters:copy(caffeparameters)
--torch_parameters = torch.load(caffeparameters)
